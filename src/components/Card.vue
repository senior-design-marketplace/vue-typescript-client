<template :id="id">
  <v-hover v-slot:default="{ hover }">
    <div style="width: 350px; height: 350px" class="mx-auto">
      <v-card
        width="350"
        min-height="350"
        :max-height="hover ? '' : '350'"
        :elevation="hover ? 12 : 2"
        :ripple="false"
        :style="hover ? 'z-index: 2; position: absolute;' : 'z-index: 1'"
      >
        <v-container class="pa-1" @click="$router.push(`/project/${id}`)" style="cursor: pointer;">
          <v-item-group>
            ...
            <v-card-text>
              <div v-if="tagline != null" v-text="tagline" :class="hover ? '' : 'line-clamp'" />
              <v-chip-group v-if="hover" column>
